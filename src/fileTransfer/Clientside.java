...
public class Clientsite extends JFrame implements ActionListener,Runnable
{
   ...
   private boolean startstatus;
    ...
    Dimension dim=new Dimension((int)(wndSize.width/1.3),(int)(wndSize.height/4));
...
//              System.out.println("location:"+driveValue+"/"+dir[i]);
					    if(!(files[i].exists())||content[i]==null||content[i].length()==0)
					    {
                result[i]="FAILURE";
						    continue;
					    }
...
//				    System.out.println("Text files published");
				    if(attach<=0)
            {
                    ...
                    model.setData(starttime.toString(),endtime.toString(),dest,result[size]);
                    continue begin;
            ...
            boolean err=false;
              ...
              if(!client.bytetransfer(driveValue+"/"+dir[j],name[j]))
              {
                err=true;
                result[j]="FAILURE";
              }
    ...
    {
     while(true)
     {  String dir=JOptionPane.showInputDialog(this,"Please enter file directory:");
        if(dir==null||dir.length()==0)
        {
          if(dir==null)
            break;
          int result=JOptionPane.showConfirmDialog(this,"You have not specifiy a directory\n"+

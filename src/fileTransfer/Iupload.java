...
public class Iupload extends JFrame implements ActionListener,Runnable
{
  ...
  JLabel jLabel1 = new JLabel();
    ...
    Dimension dim=new Dimension((int)(wndSize.width/1.3),(int)(wndSize.height/2));
...
begin:
            ...
            String[] content=web.list();
						if(content.length==0)//no meta file means no upload operation
            {
              if(!startstatus)
              continue begin;
							continue;
          ...
          meta=null;
					...
					{
						size=Integer.parseInt(fno.substring(0,attachInd));//total file number
						attach=Integer.parseInt(fno.substring(attachInd+1));//attached file number
					}
					//find out password
					...
					Timestamp endcheck=new Timestamp(seconds+600000);//10 minutes later
          ...
          server=new ServerSocket(portValue);
              ...
              server=new ServerSocket(portValue);}
							...
							addr=(add[0]&0xff)+"."+(add[1]&0xff)+"."+(add[2]&0xff)+"."+(add[3]&0xff)+
											  ":"+client.getPort();
...
//             System.out.println(password+","+passwordClient);
          ...
          data=(Vector)input.readObject();
					...
					{
            try{
						files[i]=new File(iPath+"/"+path[i]+"/"+name[i]);
            ...
            {System.out.println(e);}
					...
					data=new Vector();
					data.add(path);data.add(name);data.add(content);
					//output.
					output.writeObject(data);
                ...
                logdir.mkdir();
					      ...
					      if(!(log.exists()))
						      log.createNewFile();
					      FileWriter logWriter=new FileWriter("logfile/record.txt",true);
					      String onerecord="[IP:";
					      onerecord+=addr+",";
                ...
                model.setData(starttime.toString(),endtime.toString(),dest,reply[size]);
                client.close();
                server.close();
                server=null;
                continue begin;
...
//            System.out.println("finish sending "+name[i]);
    ...
    if(e.getSource()==start)
    {
      ...
      {
         while(true)
        {  String dir=JOptionPane.showInputDialog(this,"Please enter port number:");
          if(dir==null||dir.length()==0)
          {
            JOptionPane.showMessageDialog(this,"You have not specifiy a port number\n"+
                                                        "Please try again!");
            continue;
    ...
    if(e.getSource()==port)
    {
     ...
     {  String dir=JOptionPane.showInputDialog(this,"Please enter port number:");
        if(dir==null||dir.length()==0)
        {
          if(dir==null)
            break;
          int result=JOptionPane.showConfirmDialog(this,"You have not specifiy a port number\n"+
